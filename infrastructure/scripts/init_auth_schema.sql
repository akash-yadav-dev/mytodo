...
-- This migration creates the necessary tables for authentication and session management
...
COMMENT ON TABLE sessions IS 'Manages active user sessions and refresh tokens';
COMMENT ON COLUMN users.password_hash IS 'Bcrypt hashed password';
COMMENT ON COLUMN sessions.refresh_token IS 'JWT refresh token for obtaining new access tokens';
COMMENT ON COLUMN sessions.revoked_at IS 'Timestamp when session was revoked (for logout)';
